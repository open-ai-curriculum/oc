...
STANDARDS_MAP_PATH = REPO_ROOT / "docs" / "standards" / "ela-literacy-node-standards-map.json"
OUTPUT_YAML = REPO_ROOT / "domains" / "ela_literacy" / "standards-augmented-graph.yaml"
OUTPUT_JSON = REPO_ROOT / "domains" / "ela_literacy" / "standards-augmented-graph.json"
OUTPUT_MD = REPO_ROOT / "domains" / "ela_literacy" / "standards-augmented-graph.md"
...
def build_augmented_graph() -> dict:
    ...
    for node in standards_map.get("nodes", []):
        ...
        performance = node["performance_baseline"]
        ccss = node["ccss_ela_literacy"]
        ...
        for dependency_id in node["graph_dependencies"]:
            edges.append({"source": node_id, "target": dependency_id, "type": "DEPENDS_ON"})
        ...
        if grade_band_id not in grade_bands:
            grade_bands[grade_band_id] = {"id": grade_band_id, "label": grade_band_label, "type": "grade_band"}
        edges.append(
            {
                "source": node_id,
        ...
        performance_label = performance["minimum_expected_performance_level"]
        ...
        edges.append(
            {
                ...
                "mapping_status": performance["mapping_status"],
                "stretch_level": performance["stretch_performance_level"],
            }
        )

        ccss_anchor_label = ccss["anchor_statement"]
        ccss_anchor_id = f"ccss-anchor:{slugify(ccss['grade_or_band'])}:{slugify(ccss_anchor_label)}"
        if ccss_anchor_id not in ccss_anchors:
            ccss_anchors[ccss_anchor_id] = {
                "id": ccss_anchor_id,
                "label": ccss_anchor_label,
                "type": "ccss_anchor",
                "grade_or_band": ccss["grade_or_band"],
                "strand": ccss["strand"],
    ...
    return {
        "meta": {
            "id": "ela-literacy-standards-augmented-graph",
            "domain": "ela_literacy",
...
def write_markdown(augmented_graph: dict) -> None:
    ...
    lines = [
        "# Standards-Augmented ELA/Literacy Graph",
        "",
        "## Purpose",
        "",
        "This artifact joins the canonical ELA/literacy capability graph to grade-band and standards entities so standards alignment can participate in graph-native traversal, reporting, and mastery heat-map generation.",
        "",
        "## Sources",
        "",
        f"- `{meta['source_graph']}`",
        ...
        "## Claim Boundary",
        "",
        "This is a generated operational graph artifact. The canonical capability topology remains the ELA/literacy master knowledge graph. The standards-augmented graph exists to support traversal, reporting, and mastery/coverage analysis that require standards entities to be graph-native.",
        "",
    ]
