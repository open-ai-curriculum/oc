...
`C2` requires all of the following before `secure` status can be assigned:

1. Direct performance gate: learner reaches at least `0.90` accuracy across debugging strategy, trace reasoning, and test cases tasks.
2. Meaning gate: learner explains the targeted debugging and test reasoning ideas in at least `2` of `3` prompts.
